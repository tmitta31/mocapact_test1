...
#         # self.features_extractor_kwargs['features_dim'] = 206
...
#         processed_obs = []
#         for k in self.observable_keys:
#             if k in observations:
#                 processed_obs.append(observations[k].view(-1))
#         result = torch.cat(processed_obs, dim=-1)
#         print(f"Feature extractor output shape: {result.shape}")
#         return result
...
#                     processed_obs[key] = torch.as_tensor(obs).float().to(self.device)
#                 else:
#                     processed_obs[key] = obs
#             return processed_obs, True
#         else:
#             return super().obs_to_tensor(observation)
        
#     def extract_features(self, observations: Union[torch.Tensor, Dict[str, torch.Tensor]]) -> torch.Tensor:
#         if isinstance(observations, dict):
#             print(f"Observation keys: {observations.keys()}")
#             features = self.features_extractor(observations)
#         else:
#             observations = {"obs": observations if isinstance(observations, torch.Tensor) else torch.as_tensor(observations).float().to(self.device)}
#             features = self.features_extractor(observations)
        
#         print(f"Total features shape: {features.shape}")
#         print(f"Expected features: {self.gpt.tok_emb.in_features}")
        
#         if features.size(-1) != self.gpt.tok_emb.in_features:
#             raise ValueError(f"Feature size mismatch. Expected {self.gpt.tok_emb.in_features}, got {features.size(-1)}")
...
#     #     if isinstance(observation, dict):
#     #         B = next(iter(observation.values())).shape[0]
#     #     else:
...
#     # #         features = torch.cat([obs.view(obs.size(0), -1) for obs in observations.values()], dim=-1)
#     # #     else:
#     # #         features = observations.view(observations.size(0), -1)
        
#     # #     # Ensure the feature size matches the expected input size for the GPT model
#     # #     if features.size(-1) != self.gpt.gpt_config.obs_size:
#     # #         raise ValueError(f"Feature size mismatch. Expected {self.gpt.gpt_config.obs_size}, got {features.size(-1)}")
        
#     # #     # Add time dimension if it's missing
#     # #     if features.dim() == 2:
...
#     #     features_expanded = features.expand(-1, context.size(1), -1)

#     #     context[torch.arange(len(idx)), idx] = features_expanded[np.arange(len(idx)), 0]
